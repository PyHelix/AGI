...
def train(skill: str, weights: Path, data: Path, out: Path) -> None:
    ...
    subprocess.run(
        [sys.executable, str(script), "--weights", str(weights), "--data", str(data), "--output", str(out)],
        check=True,
    )
...
def main() -> None:
    ...
    key = bytes.fromhex(Path("project_keys/weights.key").read_text().strip())
    enc = encrypt_file(delta, key)

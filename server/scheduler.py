...
def main() -> None:
    ...
    parser.add_argument(
        "--out", default="work_units", help="Directory for generated work units"
    )
    parser.add_argument(
        "--board", default="scoreboard.csv", help="Scoreboard CSV path"
    )
    args = parser.parse_args()

    skill = pick_best_skill(Path(args.board))
    xml = create_wu(skill, Path(args.data), Path(args.out))
    print(f"created {xml} for skill {skill}")
